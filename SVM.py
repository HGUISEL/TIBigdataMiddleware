...
def SVMTrain():
    ...
    ##### 모델링 ######
    #train data load
    data=pd.read_csv("/home/dapi1/TIBigdataMiddleware/train data (svm)/topic_20100909-20200909.csv")
...
def Pre_date(date):
    ...
    return idlst, titles, tokenized_doc, contents, times
    ...
    while len(resp['hits']['hits']):
        ...
        for doc in resp['hits']['hits']:
            if "file_extracted_content" in doc["_source"].keys():
                idlst.append((doc['_id']))
                titles.append((doc["_source"]["post_title"]))
                times.append((doc["_source"]["indexed_datetime"]))    
...
def MoEs(date):
    ...
    if collection_num==0:#최초 시작
        ...
        (doc_id, doc_title, tokenized_doc, contents, times)=Pre_date(date)
        ...
        print('svmDB에 ',collection_num,'개의 데이터가 있습니다. ')
        (doc_id, doc_title, tokenized_doc, contents, times)=Post_date(date)
...
"""
log파일 보다 print를 해서 더 많이 쓰기

~월의 ~몇 개의 데이터를 불러왔습니다.
~개의 문서를 불러왔습니다.
~개의 문서에서 ~개의 명사를 추출하였습니다. 
오래 걸리는 것은 tmqp사용하여 시간 재기
